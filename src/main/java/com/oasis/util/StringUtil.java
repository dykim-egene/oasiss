...
public class StringUtil {
	...
	final public synchronized static String getLine(BufferedReader r)
		...
		while (r.ready()) {
			...
			if (line == null)
				break;
			if (line.trim().length() == 0 || line.trim().charAt(0) == DEF_SHARP) {
				continue;
			}
			return line;
		}
	...
	/**
	 * 문자열을 주어진 DEF_DELIM으로 분리
	 * @param line
	 * @return
	 */
	public synchronized static String[] getArray(String line) {
		return getArray(line, DEF_DELIM, true);
	}
	/**
	 * 문자열을 주어진 DELIM  분리
	 * @param line
	...
	public synchronized static Vector getVector(String line, char delim,
		...
		if(line == null) line = "";/* SECURE-201911 */
...
*/
	 ...
	 * @return
	 */
	public static boolean getBoolean(String v, String formatChar) {
		if(StringUtil.invalid(v)) return false;
	...
	public static String formatDateValueDT(String v, String formatChar) {
		if (StringUtil.valid(v) && v.length() >= 8) {
			return v.substring(0,4) + formatChar + v.substring(4, 6) + formatChar + v.substring(6, 8);
		}
		return v;
	}

	public static String removeFormatedDateValue(String v, String formatChar) {
		if (StringUtil.valid(v)) {
			v = v.replaceAll("-", "");

			if (StringUtil.valid(formatChar)) {
				v = v.replaceAll(formatChar, "");
			}
		}
	...
	/**
	 * http, https 프로토콜 비교 시작
	 * @param s
	 * @return
	 */
	public static boolean isHttp(String s) {

		if(StringUtil.valid(s)) return false;

		if(httpPattern.matcher(s).find()) return true;

		return false;

	}

	public static char DEF_DELIM = ',';
	public static char DEF_SHARP = '#';
	public static Pattern httpPattern = Pattern.compile("^(http|https)://*");
}